...
import {ISuperfluidGovernance, ISuperfluid, ISuperfluidToken, ISuperApp, SuperAppDefinitions, ContextDefinitions} from "../../interfaces/superfluid/ISuperfluid.sol";
...
library AgreementLibrary {
        ...
        returns (ISuperfluid.Context memory appContext, bytes memory newCtx)
    {
        ...
        if (isSuperApp && !isJailed) {
            ...
            if ((noopMask & inputs.noopBit) == 0) {
                ...
                appContext.appAllowanceUsed = max(
                    0,
                    min(
                        inputs.appAllowanceGranted.toInt256(),
                        max(
                            appContext.appAllowanceWanted.toInt256(),
    ...
    function _popCallbackStack(bytes memory ctx, int256 appAllowanceUsedDelta)
        private
        returns (bytes memory newCtx)
    {
        // app allowance params stack POP
        return
            ISuperfluid(msg.sender).appCallbackPop(ctx, appAllowanceUsedDelta);
    }
